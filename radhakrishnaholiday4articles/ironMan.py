...
def jaau(x, y): #origin of the pen
    pen.penup()
    pen.goto(x-x_offset,(y*-1)+y_offset)
    pen.pendown()  


def chitra(coord,co=(0,0,0)): #start the chitraing
    pen.color(co) #color of the pen
    t_x,t_y = coord[0] # see the coordinates
    jaau(t_x,t_y) #run the go function
    pen.fillcolor(co) #fill
    pen.begin_fill()
    t = 0
    for i in coord[1:]: #loop for fill
        print(i)
        x,y = i
        if t:
            jaau(x,y)
...
def banaau_fn(coord,mode = 1,co = (0,0,0),thickness = 1):
    ...
    if mode:
        ...
        chitra(coord=coord,co = co)


def banaau(retain=True): # the main function
    banaau_fn(dress,mode = 0)
    banaau_fn(face,mode = 0)
    banaau_fn(iface,co = (255,255,255),mode = 0)
    banaau_fn(mookh,mode = 0)
    banaau_fn(nose,mode = 0)
    banaau_fn(ebrow,mode = 0)
    banaau_fn(lines,mode = 1,thickness=2)
    banaau_fn(eyes,mode = 0)
    banaau_fn(eball,mode = 0,co = (255,255,255))
    if retain:
        tu.done()
